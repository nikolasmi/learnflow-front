<template>
  ...
  >
    <li class="px-4 py-2 cursor-default">
      Korisnik: <span class="font-semibold">{{ userStore.user?.identity }}</span>
    </li>
    <li class="flex flex-col">
      ...
      >
        <IconProfile class="fill-primary dark:fill-dark-primary" /> Profil
      </RouterLink>
      <RouterLink
        ...
        @click="emit('closeMenu', false)"
      >
        <IconChangePassword class="fill-primary dark:fill-dark-primary" /> Promeni šifru
      </RouterLink>
      <button
        @click="handleLogout"
        class="text-left py-2 px-4 text-primary dark:text-dark-primary hover:bg-primary/[.10] cursor-pointer flex gap-x-2"
      >
        <IconLogout class="fill-primary dark:fill-dark-primary" /> Izloguj se
      </button>
    </li>
  </ul>
</template>
